function nlp = rigidImpactConstraint(obj, nlp, src, tar, bounds, varargin)
    ...
    addNodeConstraint(nlp, obj.xMap, {'x','xnImp'}, 'first', 0, 0, 'Linear');
    cstr_name = fieldnames(obj.ImpactConstraints);
    
    % Add identity for continuous node
    x = obj.States.x;
    xn = obj.States.xn;
    dx = obj.States.dx;
    dxn = obj.States.dxn;
    xcont = SymFunction('xcont',x-xn,{x,xn});
    dxcont = SymFunction('dxcont',dx-dxn,{dx,dxn});
    addNodeConstraint(nlp, xcont, {'x','xn'}, 'first', 0, 0, 'Linear');
    addNodeConstraint(nlp, dxcont, {'dx','dxn'}, 'first', 0, 0, 'Linear');
    
    
    % the velocities determined by the impact constraints
    if isempty(cstr_name)
        % by default, identity map
        addNodeConstraint(nlp, obj.dxMap{1}, {'dx','dxnImp'}, 'first', 0, 0, 'Linear');
    else
        vars   = nlp.OptVarTable;
        dep_vars = [vars.dx(1);vars.xnImp(1);vars.dxnImp(1)];
